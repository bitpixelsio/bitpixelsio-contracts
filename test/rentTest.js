...
describe('RentTest', async function () {
  ...
  it("claimTest", async () => {
    ...
    result = await instance.connect(accounts[1]).claim(1, 3, 3, 2, {value: ethers.utils.parseEther((6*price).toString()) })
    ...
    result = await instance.connect(accounts[2]).claim(5, 7, 1, 3, {value:  ethers.utils.parseEther((3*price).toString())})
    ...
    result = await instance.connect(accounts[3]).claim(50, 55, 10, 3, {value: ethers.utils.parseEther((30*price).toString())})
    supply = parseInt(await instance.totalSupply(), 10);
    ...
    let autResult = 0;
    try{
      await instance.connect(accounts[4]).claim(52, 56, 6, 1, {value: ethers.utils.parseEther((6*price).toString())})
    }catch (e) {
      // console.log(e, 1)
      autResult = 1
    }
  ...
  it("rentTest", async () => {
    ...
    await instance.connect(accounts[1]).listRenting(util.getPixelIds(1,3,3,2), ethers.utils.parseEther("2"), util.dateToInput(startDate), util.dateToInput(endDate),
      minDaysToRent, maxDaysToRent, minDaysBeforeRentCancel, weeklyDiscount * 10, monthlyDiscount * 10, yearlyDiscount * 10)
    let data = (await readerInstance.getRentData(util.getPixelIds(1,3,3,2)[0]))[0]
    ...
    await instance.connect(accounts[1]).cancelRentListing([util.getPixelIds(1,3,3,2)[0]], util.dateToInput(startDate), util.dateToInput(endDate))
    await instance.connect(accounts[1]).listRenting([util.getPixelIds(1,3,3,2)[0]], ethers.utils.parseEther(rentDailyPrice.toString()), util.dateToInput(startDate), util.dateToInput(endDate),
      minDaysToRent, maxDaysToRent, minDaysBeforeRentCancel, weeklyDiscount * 10, monthlyDiscount * 10, yearlyDiscount * 10)
    ...
    result = await instance.connect(accounts[2]).rentPixels([util.getPixelIds(1,3,3,2)[0]], util.dateToInput(startDate), util.dateToInput(endDate),
    ...
    data = (await readerInstance.getRentData(util.getPixelIds(1,3,3,2)[0]))[0]
    assert.equal(Math.floor((balanceAddr2 - balance2Addr2 - gasUsed * gasPrice) / weiHalf), Math.floor(ethers.utils.parseEther(totalRent.toString()) / weiHalf), 'rent not paid')
    ...
    result = await instance.connect(accounts[2]).cancelRent([util.getPixelIds(1,3,3,2)[0]], util.dateToInput(startDate), util.dateToInput(endDate))
    trans = await ethers.provider.getTransactionReceipt(result.hash)
    gasUsed = trans.gasUsed.toNumber();
    gasPrice = result.gasPrice.toNumber();
    const balance3Addr2 = await ethers.provider.getBalance(accounts[2].address);
    data = (await readerInstance.getRentData(util.getPixelIds(1,3,3,2)[0]))[0]
    assert.equal(Math.floor((balance3Addr2 - balance2Addr2 + gasUsed * gasPrice) / weiHalf), Math.floor(ethers.utils.parseEther(totalRent.toString()) / weiHalf), 'rent not cancelled back')
    ...
    await instance.connect(accounts[2]).rentPixels([util.getPixelIds(1,3,3,2)[0]], util.dateToInput(startDate), util.dateToInput(endDate),
      {value: ethers.utils.parseEther(totalRent.toString())} )
    data = (await readerInstance.getRentData(util.getPixelIds(1,3,3,2)[0]))[0]
    ...
    for(let i = 0; i < 60; i++){
      ...
      if(now2 < now + startTimeDelay){
        ...
        if(now2 - now - startTimeDelay == claimRentDay){
          assert.equal(Math.floor(ethVal * 10000), Math.floor(rentDailyPrice * (now2 - now - startTimeDelay) * (100 - weeklyDiscount) * 100), 'tenant problem')
          ...
          assert.equal(Math.floor((balance2Addr1 - balanceAddr1 + gasUsed * gasPrice) / weiHalf), Math.floor(claim * (claimRentDay + 1) / claimRentDay / weiHalf), 'rent not claimed')
          data = (await readerInstance.getRentData(util.getPixelIds(1,3,3,2)[0]))[0]
          assert.equal(data.rentCollectedDays, claimRentDay + 1, 'rent claim not saved')
        }else if(now2 - now - startTimeDelay < claimRentDay){
          assert.equal(Math.floor(ethVal * 10000), Math.floor(rentDailyPrice * (now2 - now - startTimeDelay) * (100 - weeklyDiscount) * 100), 'tenant problem')
        }else{
          assert.equal(Math.floor(ethVal * 10000), Math.floor(rentDailyPrice * (now2 - now - startTimeDelay - claimRentDay - 1) * (100 - weeklyDiscount) * 100), 'tenant problem')
        }
      }else{
        break;
      }
